...
import sentinel_config as config  # pylint:disable=import-error
import sentinel_helpers as helpers


class SentinelScriptLogHandler(logging.Handler):

    def __init__(self, parent_wrapper, settings, log_level=None):
        super(SentinelScriptLogHandler, self).__init__()
    ...
    def _setup_logging(self, log_level):
        ...
        formatter = SentinelLoggerFactory.create_formatter()
...
class SentinelFileLogWrapper(object):
    ...
    def _enable_logging(self, log_level):
        ...
        file_log_handler = SentinelLoggerFactory.create_file_log_handler(
            log_level
        )
...
class SentinelLogWrapper(object):
    """
    ...
    def _setup_script_logging(self, log_level):
        self.script_log_handler = SentinelScriptLogHandler(
    ...
    def _setup_file_logging(self, log_level):
        ...
        self.file_log_handler_wrapper = SentinelFileLogWrapper(
            self.settings, self.logger, log_level
        )


class SentinelLoggerFactory(object):
    ...
    def init_logging(cls, parent_wrapper, settings):
        ...
        Initializes logging.
        """
        wrapper = SentinelLogWrapper(parent_wrapper, settings)
