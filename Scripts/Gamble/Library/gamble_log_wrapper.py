...
import gamble_config as config  # pylint:disable=import-error
import gamble_helpers as helpers


class GambleScriptLogHandler(logging.Handler):

    def __init__(self, parent_wrapper, settings, log_level=None):
        super(GambleScriptLogHandler, self).__init__()
    ...
    def _setup_logging(self, log_level):
        ...
        formatter = GambleLoggerFactory.create_formatter()
...
class GambleFileLogWrapper(object):
    ...
    def _enable_logging(self, log_level):
        ...
        file_log_handler = GambleLoggerFactory.create_file_log_handler(
            log_level
        )
...
class GambleLogWrapper(object):
    ...
    def _setup_script_logging(self, log_level):
        self.script_log_handler = GambleScriptLogHandler(
    ...
    def _setup_file_logging(self, log_level):
        ...
        self.file_log_handler_wrapper = GambleFileLogWrapper(
            self.settings, self.logger, log_level
        )


class GambleLoggerFactory(object):
    ...
    def init_logging(cls, parent_wrapper, settings):
        ...
        Initializes logging.
        """
        wrapper = GambleLogWrapper(parent_wrapper, settings)
