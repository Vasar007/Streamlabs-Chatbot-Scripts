...
import queue_config as config  # pylint:disable=import-error
import queue_helpers as helpers


class QueueScriptLogHandler(logging.Handler):

    def __init__(self, parent_wrapper, settings, log_level=None):
        super(QueueScriptLogHandler, self).__init__()
    ...
    def _setup_logging(self, log_level):
        ...
        formatter = QueueLoggerFactory.create_formatter()
...
class QueueFileLogWrapper(object):
    ...
    def _enable_logging(self, log_level):
        ...
        file_log_handler = QueueLoggerFactory.create_file_log_handler(
            log_level
        )
...
class QueueLogWrapper(object):
    ...
    def _setup_script_logging(self, log_level):
        self.script_log_handler = QueueScriptLogHandler(
    ...
    def _setup_file_logging(self, log_level):
        ...
        self.file_log_handler_wrapper = QueueFileLogWrapper(
            self.settings, self.logger, log_level
        )


class QueueLoggerFactory(object):
    ...
    def init_logging(cls, parent_wrapper, settings):
        ...
        Initializes logging.
        """
        wrapper = QueueLogWrapper(parent_wrapper, settings)
